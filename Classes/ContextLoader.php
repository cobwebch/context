...
class ContextLoader
{
     ...
     */
    protected $extensionKey = 'context';
    ...
    {
        $context = array();
        $typoscriptKey = 'tx_' . $this->extensionKey . '.';
        // Check for existing context information
        if (isset($params['config'][$typoscriptKey])) {
            $contextSetup = GeneralUtility::removeDotsFromTS($params['config'][$typoscriptKey]);
            ...
            if (count($contextSetup) > 0) {
                ...
                if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF'][$this->extensionKey]['contextStorage'])) {
                    foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF'][$this->extensionKey]['contextStorage'] as $className) {
